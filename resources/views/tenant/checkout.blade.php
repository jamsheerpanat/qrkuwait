...
@section('content')
    <div x-data="checkoutState()" x-init="init()" class="pb-32 max-w-2xl mx-auto">
        <div class="mb-10 mt-8">
            <h1 class="text-4xl font-black italic text-slate-900 mb-2">Checkout</h1>
            <p class="text-slate-500 font-bold uppercase tracking-widest text-xs">Complete your order</p>
        </div>
        ...
        <form action="{{ route('tenant.checkout.store', $tenant->slug) }}" method="POST" id="checkoutForm">
            ...
            <div class="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-xl border-t z-50">
                <div class="max-w-2xl mx-auto flex items-center justify-between gap-6">
                    <div>
                        ...
                        <p class="text-3xl font-black italic text-slate-900"><span x-text="total.toFixed(3)"></span> <span
                                class="text-base not-italic text-slate-400 font-bold">KWD</span></p>
                    </div>
                    <button type="submit"
                        class="bg-slate-900 text-white px-10 py-5 rounded-[2rem] font-black italic text-xl shadow-2xl shadow-slate-900/40 transform active:scale-95 transition">
                        Place Order
                    </button>
                </div>
            </div>
