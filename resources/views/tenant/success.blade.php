...
@section('content')
        @php
            $isDineIn = $order->delivery_type === 'dine_in';
        @endphp
        ...
        <div class="py-20 flex flex-col items-center text-center max-w-lg mx-auto" x-data="{ 
            ...
            @if($isDineIn)
                {{-- DINE-IN SUCCESS --}}
                <div
                    class="w-32 h-32 bg-gradient-to-br from-amber-100 to-orange-100 text-orange-500 rounded-[3rem] flex items-center justify-center mb-10 relative">
                    <span class="text-6xl animate-bounce">🍳</span>
                    <div class="absolute -right-2 -top-2 w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                ...
                <h1 class="text-4xl font-black italic text-slate-900 mb-4">Sent to Kitchen!</h1>
                <p class="text-slate-500 font-bold mb-6 leading-relaxed text-lg">
                    Your order <span class="text-amber-600 font-black">#{{ $order->order_no }}</span> is being prepared.
                </p>

                @if($order->table_number)
                    <div
                        class="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-2xl px-6 py-4 mb-10 inline-flex items-center gap-3">
                        <span class="text-3xl">🍽️</span>
                        <div class="text-left">
                            <p class="text-sm text-amber-700 font-medium">Serving at</p>
                            <p class="text-xl font-black text-amber-900">Table {{ $order->table_number }}</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="w-full space-y-6">
                    ...
                    <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 text-left">
                        <h3 class="text-lg font-bold text-slate-900 mb-6">Order Progress</h3>
                        <div class="space-y-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="font-bold text-slate-900">Order Received</p>
                                    <p class="text-sm text-slate-500">{{ $order->created_at->format('H:i') }}</p>
                                </div>
                            </div>
                            <div class="ml-5 w-0.5 h-6 bg-gradient-to-b from-green-500 to-amber-500"></div>
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center animate-pulse">
                                    <span class="text-lg">🍳</span>
                                </div>
                                <div>
                                    <p class="font-bold text-amber-600">Preparing</p>
                                    <p class="text-sm text-slate-500">Your food is being cooked</p>
                                </div>
                            </div>
                            <div class="ml-5 w-0.5 h-6 bg-slate-200"></div>
                            <div class="flex items-center gap-4 opacity-50">
                                <div class="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center">
                                    <span class="text-lg">🍽️</span>
                                </div>
                                <div>
                                    <p class="font-bold text-slate-400">Served</p>
                                    <p class="text-sm text-slate-400">Ready to enjoy!</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Add More Items --}}
                    <a href="{{ route('tenant.public', $tenant->slug) }}?table={{ $order->table_number }}"
                        class="flex items-center justify-center gap-4 w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white p-6 rounded-[2.5rem] font-black italic text-xl shadow-xl shadow-amber-200 transform active:scale-95 transition">
                    ...
                    {{-- Call Waiter / Request Bill (Future Feature) --}}
                    <div class="grid grid-cols-2 gap-4">
                        <button onclick="alert('Feature coming soon!')"
                            class="flex flex-col items-center justify-center gap-2 w-full bg-slate-100 text-slate-600 p-6 rounded-2xl font-bold hover:bg-slate-200 transition">
                            <span class="text-2xl">🙋</span>
                            <span class="text-sm">Call Waiter</span>
                        </button>
                        <button onclick="alert('Feature coming soon!')"
                            class="flex flex-col items-center justify-center gap-2 w-full bg-slate-100 text-slate-600 p-6 rounded-2xl font-bold hover:bg-slate-200 transition">
                            <span class="text-2xl">🧾</span>
                            <span class="text-sm">Request Bill</span>
                        </button>
                    </div>
                </div>

            @else
                {{-- DELIVERY/PICKUP SUCCESS --}}
                <div
            ...
            <div class="mt-12 w-full p-8 bg-white rounded-[3rem] border border-slate-100 text-left">
                <h4 class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-6 border-b pb-4">
                    @if($isDineIn)
                        Your Items
                    @else
                        Order Details
                ...
                @if($isDineIn)
                    {{-- Dine-In: Show Items List --}}
                    <div class="space-y-4 mb-6">
                        @foreach($order->items as $item)
                            <div class="flex justify-between items-center">
                                <div class="flex items-center gap-3">
                                    <span
                                        class="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center text-sm font-black text-amber-700">{{ (int) $item->qty }}</span>
                                    <span class="font-bold text-slate-800">{{ $item->item_name }}</span>
                                </div>
                                <span class="font-bold text-slate-600">{{ number_format($item->line_total, 3) }} KWD</span>
                            </div>
                        @endforeach
                    </div>
                    <div class="border-t pt-4 flex justify-between items-center">
                        <span class="font-bold text-slate-500">Total</span>
                        <span class="text-xl font-black text-slate-900">{{ number_format($order->total, 3) }} KWD</span>
                    </div>
                @else
                    {{-- Delivery/Pickup: Show Order Info --}}
                    <div class="space-y-4">
                        @if($order->table_number)
            ...
            @if(!$isDineIn)
                <!-- Share Receipt via WhatsApp (Only for delivery/pickup) -->
                @php
